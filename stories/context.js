...
export default function(items: any) {
  return {
            ...
            {Object.keys(items).map(itemKey => {
              ...
              if (childrenProps.hidden)
                return (
                  <Alert
                    key={itemKey}
                    message={`${childrenProps.refId.toString()} ${itemKey} is hidden.`}
                    type="warning"
                  />
                );
              return (
                <Alert
                  key={itemKey}
                  message={`render ${childrenProps.refId.toString()} ${itemKey} is show.`}
                  type="success"
                />
              );
            })}
    ...
    renderComponent: (refId: RefId) =>
      `this is the component at '${refId.toString()}'`,
    refId: new RefId(""),
    routes: []
  };
}
